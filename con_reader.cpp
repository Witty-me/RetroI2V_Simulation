...
void get_inrange_tags_uplink(reader_t reader, vector<tag_t> &inrange_tags)
{ // used for carrier sensing.
    for (int i = 0; i < tags.size(); i++)
    {
...
void tag_handle(tag_t *tag, reader_t *reader)
{
    ...
    {
        if (tag->input.overhear_count)
        { // handle overhearing ack
            tag_set_reader_quiet(tag);
        }
        if (tag->input.ack_flag == ACK_FLAG)
        { // handle listening ack
            uint32_t alias = 0;
        ...
        if (tag_check_reader_quiet(tag, tag->input.srcAddr) == 0)
        { // assign
            uint8_t shortAddr = 0;
            uint32_t alias = 0;
            alias = tag->input.srcAddr;
...
//    usleep(1000);
    ...
    case DOWNLINK_SEND_END:
    {
        ...
        for (int i = readers[check_index].collisions.size() - 1; i >= 0; i--)
        { // for each potenial collision.
    ...
    case UPLINK_SEND_START:
    {
        ...
        if (tags[check_index].output.type == DISC_RESPONSE)
        { // send disc ack.
        ...
        else if (tags[check_index].output.type == QUERY_RESPONSE)
        { // send preamble + payload.
    ...
    case UPLINK_SEND_DISC_ACK_END:
    {
        ...
        if (tags[check_index].output.srcAddr == 0)
        { // send preamble and payload following disc ack response.
...
#else
#if TEST_DEF == 1
    {500, UPLINK_MAX_DISTANCE_TYPE3_B500, 2, 1 + CUSTOM_OFFSET_INDEX, 1},
#elif TEST_DEF == 2
    {500, UPLINK_MAX_DISTANCE_TYPE3_B500, 2, 1 + CUSTOM_OFFSET_INDEX, 2},
#elif TEST_DEF == 3
    {500, UPLINK_MAX_DISTANCE_TYPE3_B500, 2, 1 + CUSTOM_OFFSET_INDEX, 4},
#elif TEST_DEF == 4
    {500, UPLINK_MAX_DISTANCE_TYPE3_B500, 1, 0 + CUSTOM_OFFSET_INDEX, 2},
#elif TEST_DEF == 5
    {500, UPLINK_MAX_DISTANCE_TYPE3_B500, 3, 2 + CUSTOM_OFFSET_INDEX, 2},
#endif
#endif
    {0, 0, 0, 0, 0}};
